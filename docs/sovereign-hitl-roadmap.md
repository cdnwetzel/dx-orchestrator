...
- **A narrow guard was the bug, twice (2026-09-14).** The role-card count guard was widened from README-only to a hardcoded two-file tuple that instructed the next author to extend it — so `RELEASE_READINESS.md` sat at 38 against a 40-card deck, and two other docs stated 40 by luck rather than enforcement. It now reads every tracked `*.md`. Same shape as the 0.7.2 red-line lesson and the evidence-store guard above: **a check that needs manual extension is one forgotten edit from silence.**
